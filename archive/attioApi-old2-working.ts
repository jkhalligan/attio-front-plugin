...
export async function getCompany(recordId: string): Promise<AttioCompany> {
  try {
    ...
    console.log('🏢 Company fetched - full response:', response);
    console.log('🏢 Company data object:', response.data);
    console.log('🏢 Company name from values:', response.data?.values?.name?.[0]?.value);
